...
class SolicitacaoProcedimentoAmbulatorial(Base):
    __tablename__ = "solicitacao_procedimento_ambulatorial"

    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("user.id"), nullable=True)
    user = relationship("User", back_populates="solicitacao_procedimento")
