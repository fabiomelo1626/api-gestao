...
@nascido_vivo.get("/nascidos-vivos", response_model=NascidoVivoResponse)
...
@nascido_vivo.get("/nascidos-by-local_id/{local_id}", response_model=NascidoVivoResponse)
def search_nascido_vivo_local(local_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    nascidos = db.query(NascidoVivo).filter(NascidoVivo.local_id == local_id).first()
    if not nascidos:
        HTTPException(status_code=404, detail="Nascido vivo não encontrado para o local")
    return nascidos
